...
test.describe('Accessibility (axe-core)', () => {
  test('hub page has no critical/serious a11y violations', async ({
    page,
    createProfile,
  }) => {
  ...
  test('profile creation page has no critical/serious a11y violations', async ({
    page,
  }) => {
  ...
  test('settings page has no critical/serious a11y violations', async ({
    page,
    createProfile,
  }) => {
    await createProfile({ name: 'A11yKid', age: '7' });
    ...
    const serious = filterViolations(results.violations);
    expect(serious, `Settings a11y violations: ${JSON.stringify(serious, null, 2)}`).toHaveLength(0);
  });

  test('rewards page has no critical/serious a11y violations', async ({
    page,
    createProfile,
  }) => {
    await createProfile({ name: 'A11yKid', age: '7' });
  ...
  test('Memory Match mid-game has no critical/serious a11y violations', async ({
    ...
    await expect(
      page.locator('[aria-label="Memory card grid"]'),
    ).toBeVisible();
    ...
    const serious = filterViolations(results.violations);
    expect(serious, `Memory Match a11y violations: ${JSON.stringify(serious, null, 2)}`).toHaveLength(0);
  });
  ...
  test('parental dashboard gate has no critical/serious a11y violations', async ({
    ...
    const serious = filterViolations(results.violations);
    expect(serious, `Parental gate a11y violations: ${JSON.stringify(serious, null, 2)}`).toHaveLength(0);
  });
});

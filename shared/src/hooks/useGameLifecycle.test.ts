...
function createMockPlugin(overrides?: Partial<GamePlugin>): GamePlugin {
  return {
    manifest: {
      id: 'test-game', name: 'Test', description: 'A test game', thumbnail: '/test.png',
      ageRange: [3, 12], skills: ['memory'], version: '1.0.0', entryPoint: '/games/test',
      minDifficulty: 1, maxDifficulty: 5, estimatedPlayTime: 5, offlineCapable: true,
      status: 'active', releaseDate: '2026-01-01', tags: [],
    },
    ...
    onEnd: vi.fn().mockReturnValue({
      gameId: 'test-game', score: 100, maxScore: 100, timeSpent: 60,
      difficulty: 1, completedAt: new Date().toISOString(), metrics: {},
...
const mockConfig: GameConfig = {
  difficulty: 1,
  profile: {
    id: 'p1', name: 'Test Kid', avatar: 'owl', age: 6, ageTier: 'junior',
    createdAt: '2026-01-01', parentPin: 'hashed',
...
describe('useGameLifecycle', () => {
  ...
  it('transitions LOADED → PLAYING on start()', async () => {
    ...
    const { result } = renderHook(() => useGameLifecycle(plugin));
    await act(async () => { await result.current.load(); });
    act(() => { result.current.start(mockConfig); });
  ...
  it('transitions PLAYING → COMPLETED on end() and returns GameResult', async () => {
    ...
    act(() => { gameResult = result.current.end(); });
